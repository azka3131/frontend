...
export function AdminLayout({
  ...
  // --- LOGIKA PENGAMAN URL GAMBAR & NAMA SEKOLAH ---
  const rawLogo = schoolSettings?.logo
  const logoSrc =
    rawLogo && rawLogo.trim() !== ''
      ? rawLogo.trim().startsWith('http')
        ? rawLogo.trim()
        : `http://127.0.0.1:8000${rawLogo.trim()}`
      : logo

  const rawName = schoolSettings?.name
  const schoolName =
    rawName && rawName.trim() !== '' ? rawName.trim() : 'SDN Dukuhbenda 02'
  // -------------------------------------------------
